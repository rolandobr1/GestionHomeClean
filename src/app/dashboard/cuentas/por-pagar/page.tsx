...
const PaymentForm = ({
  ...
  const { addPaymentToExpense } = useAppData();
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await addPaymentToExpense(expense.id, {
    ...
    <form onSubmit={handleSubmit}>
      <div className="space-y-4 py-4">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-muted-foreground">Suplidor</p>
            <p className="font-semibold">{useAppData().suppliers.find(s => s.id === expense.supplierId)?.name || 'N/A'}</p>
          </div>
          <div className="text-right">
            <p className="text-muted-foreground">Registro Nº</p>
            <p className="font-semibold">{expense.id.slice(-6).toUpperCase()}</p>
          </div>
        </div>
        <Card className="p-4 bg-muted/50">
          <div className="flex justify-between items-center">
            <div className="text-muted-foreground">Saldo Pendiente</div>
            <div className="text-2xl font-bold">RD${expense.balance.toFixed(2)}</div>
          </div>
        </Card>
