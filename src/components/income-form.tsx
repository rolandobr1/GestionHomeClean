...
export const IncomeForm = ({ income = null, onSave, onClose }: IncomeFormProps) => {
    ...
    const form = useForm<IncomeFormValues>({
      ...
      defaultValues: {
        ...
        date: income ? format(new Date(income.date + 'T00:00:00'), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd'),
    ...
    const handleAddProduct = () => {
        let newProduct: SoldProduct | null = null;
        
        if (currentProduct === 'generic') {
            if (Number(genericProductPrice) <= 0 || currentQuantity <= 0) {
                toast({ variant: "destructive", title: "Datos Inválidos", description: "Por favor, ingresa un precio y cantidad mayor a cero." });
                return;
            }
            newProduct = {
            ...
            const existingProductIndex = soldProducts.findIndex(p => p.productId === product.id && p.price === price);
    ...
    const handleSubmit = async (values: IncomeFormValues) => {
        if (values.products.length === 0) {
            form.setError("products", { type: "manual", message: "Debes agregar al menos un producto." });
            return;
        }
        setIsSaving(true);
        try {
                                                ...
                                                <TableBody>
                                                    {soldProducts.map((p, index) => (
                                                        <TableRow key={p.productId + index}>
                                                            ...
                                                            <TableCell>
                                                                <Button variant="ghost" size="icon" type="button" onClick={() => handleRemoveProduct(p.productId)} disabled={isSaving}>
                                            ...
                                            {soldProducts.map((p, index) => (
                                                <div key={p.productId + index} className="rounded-lg border bg-card text-card-foreground shadow-sm p-3 space-y-2">
                                                    <div className="flex justify-between items-start">
                                                        ...
                                                        <Button variant="ghost" size="icon" type="button" className="h-7 w-7 flex-shrink-0" onClick={() => handleRemoveProduct(p.productId)} disabled={isSaving}>
                                                            <Trash2 className="h-4 w-4 text-destructive"/>
                                                        </Button>
                                                    </div>
